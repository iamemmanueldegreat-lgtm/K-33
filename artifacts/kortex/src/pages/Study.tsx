      ...
      <main className="flex-1 h-full overflow-y-auto custom-scrollbar relative">
              ...
              ">
                {activeTab === 'Explanation' && (
                   <div className="animate-in fade-in duration-300">
                     <Markdown>{ensureString(content)}</Markdown>
                   </div>
                )}
                {activeTab === 'Key Takeaways' && (
                   keyTakeaways ? (
                     <div className="animate-in fade-in duration-300">
                       <Markdown>{ensureString(keyTakeaways)}</Markdown>
                     </div>
                   ) : (
        ...
        {!loading && !isGenerating && content && (
          <button
            ...
            className="fixed right-6 bottom-6 z-40 bg-primary hover:bg-primary/95 text-primary-foreground font-sans font-bold shadow-lg hover:scale-105 active:scale-95 transition-all duration-300 rounded-full py-3.5 px-5.5 flex items-center gap-2.5 group cursor-pointer border border-primary/10"
          >
            <Sparkles size={16} className="animate-pulse group-hover:rotate-12 transition-transform duration-300" />
