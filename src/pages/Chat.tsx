...
export default function Chat() {
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      // Compute the height (max-height of approx 8 lines, which is around 196px based on a 24px line height)
      const scrollHeight = textareaRef.current.scrollHeight;
      textareaRef.current.style.height = `${Math.min(scrollHeight, 196)}px`;
    }
  }, [input]);
          ...
          messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className={`flex flex-col max-w-[85%] min-w-0 w-full ${isUser ? 'items-end' : 'items-start pt-1'}`}>
                  <div className={`text-[15px] leading-relaxed w-full min-w-0 ${
                    isUser 
                      ? 'bg-surface px-5 py-3 rounded-3xl text-text font-normal border border-border/40 shadow-sm' 
                      : 'text-text font-normal'
                  }`}>
                    <div className={`markdown-body break-words prose prose-sm dark:prose-invert max-w-none w-full min-w-0 overflow-x-hidden ${isUser ? '[&>p]:mb-0 border-none' : 'prose-p:leading-relaxed prose-pre:overflow-x-auto prose-pre:max-w-full prose-pre:bg-surface prose-pre:border prose-pre:border-border prose-pre:my-4'}`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-1.5 pb-2 sm:p-2 sm:pb-3 bg-gradient-to-t from-background via-background to-transparent pt-12 pointer-events-none">
        ...
        <div className="pointer-events-auto w-full px-4 sm:px-6">
          <form 
            ...
            className="relative flex items-end bg-surface border border-border rounded-[28px] p-1.5 shadow-sm focus-within:ring-1 focus-within:ring-border/50 transition-all mb-0.5"
          >
            <button
              type="button"
              className="w-10 h-10 flex items-center justify-center text-text rounded-full hover:bg-background transition-colors flex-shrink-0 ml-0.5 mb-0.5"
            >
            ...
            <textarea
              ...
              className="flex-1 bg-transparent border-none pl-2 pr-4 py-2.5 text-[15px] focus:outline-none focus:ring-0 w-full placeholder:text-muted resize-none max-h-[196px] min-h-[40px] overflow-y-auto leading-relaxed scrollbar-none align-bottom"
              style={{ height: 'auto' }}
            />
